...
<body>
    ...
    <table>
        ...
        {''.join(f"<tr><td>{_html.escape(str(name))}</td><td>{dur:.2f}</td></tr>" for name, dur in summary['slowest_operations'])}
    </table>
    
    <h2>API Calls</h2>
    <table>
        <tr><th>Endpoint</th><th>Method</th><th>Duration (ms)</th><th>Status</th></tr>
        {''.join(f"<tr><td>{_html.escape(str(a.endpoint))}</td><td>{_html.escape(str(a.method))}</td><td>{a.duration_ms:.2f}</td><td>{a.status_code}</td></tr>" for a in cls._api_calls[:20])}
    </table>
    
    <h2>Streaming</h2>
    <table>
        <tr><th>Name</th><th>TTFT (ms)</th><th>Total (ms)</th><th>Chunks</th><th>Tokens</th></tr>
        {''.join(f"<tr><td>{_html.escape(str(s.name))}</td><td>{s.ttft_ms:.2f}</td><td>{s.total_ms:.2f}</td><td>{s.chunk_count}</td><td>{s.total_tokens}</td></tr>" for s in cls._streaming[:20])}
    </table>
</body>
</html>'''
        return html
