"""Backward-compatibility shim for :mod:`praisonai.cli.commands.agent`.

The implementation moved to :mod:`praisonai_code.cli.commands.agent` as part
...
* ``from praisonai.cli.commands.agent import X`` keeps working, and
* ``unittest.mock.patch("praisonai.cli.commands.agent.X")`` patches the very
  same module object that the implementation executes against.
"""

import sys as _sys

from praisonai_code.cli.commands import agent as _impl
