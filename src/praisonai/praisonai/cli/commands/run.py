"""Backward-compatibility shim for :mod:`praisonai.cli.commands.run`.

The implementation moved to :mod:`praisonai_code.cli.commands.run` as part
...
* ``from praisonai.cli.commands.run import X`` keeps working, and
* ``unittest.mock.patch("praisonai.cli.commands.run.X")`` patches the very
  same module object that the implementation executes against.
"""

import sys as _sys

from praisonai_code.cli.commands import run as _impl
