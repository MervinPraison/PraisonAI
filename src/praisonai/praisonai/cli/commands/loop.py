"""Backward-compatibility shim for :mod:`praisonai.cli.commands.loop`.

The implementation moved to :mod:`praisonai_code.cli.commands.loop` as part
...
* ``from praisonai.cli.commands.loop import X`` keeps working, and
* ``unittest.mock.patch("praisonai.cli.commands.loop.X")`` patches the very
  same module object that the implementation executes against.
"""

import sys as _sys

from praisonai_code.cli.commands import loop as _impl
