...
class _DictLike:
    """Base class that provides dict-like access to attributes."""
...
class MockLLMResponse:
    ...
    def create_tool_call_response(tool_name, arguments, tool_call_id="call_123", provider="openai"):
        ...
        class MockMessage(_DictLike):
            def __init__(self):
                self.content = ""
                self.tool_calls = [MockToolCall()]
        
        class MockChoice(_DictLike):
            def __init__(self):
                self.message = MockMessage()
        
        class MockResponse(_DictLike):
            def __init__(self):
    ...
    @staticmethod
    def create_streaming_tool_call_response(tool_name, arguments, tool_call_id="call_123"):
        """Create a streaming-compatible tool call response."""
        class MockDelta(_DictLike):
            def __init__(self):
                self.content = ""
                self.tool_calls = [Mock()]
                self.tool_calls[0].function = Mock()
                self.tool_calls[0].function.name = tool_name
                self.tool_calls[0].function.arguments = json.dumps(arguments) if isinstance(arguments, dict) else arguments
                self.tool_calls[0].id = tool_call_id
...
class TestSequentialToolCalling:
        ...
        def streaming_side_effect(*args, **kwargs):
            ...
            if any(msg.get('role') == 'tool' for msg in messages):
                ...
                if len(tool_messages) == 1:
                    return MockLLMResponse.create_streaming_tool_call_response(
                        "multiply",
                        {"a": 100, "b": 2},
                        "call_002"
                    )
    ...
    @patch('litellm.completion')
    def test_sequential_with_gemini(self, mock_litellm):
    ...
    @patch('litellm.completion')
    def test_sequential_with_ollama(self, mock_litellm):
        """Test sequential tool calling with Ollama format."""
        class OllamaToolCall(_DictLike):
        ...
        class OllamaMessage(_DictLike):
            def __init__(self, tool_calls=None, content=""):
                self.tool_calls = tool_calls
                self.content = content
        
        class OllamaChoice(_DictLike):
            def __init__(self, message):
                self.message = message
        
        class OllamaResponse(_DictLike):
            def __init__(self, message):
    ...
    def test_multiple_tools_single_response(self, mock_completion):
        ...
        class MultiToolMessage(_DictLike):
            def __init__(self):
                tool1 = Mock()
        ...
        class MultiToolChoice(_DictLike):
            def __init__(self):
                self.message = MultiToolMessage()
        
        class MultiToolResponse(_DictLike):
            def __init__(self):
    ...
    def test_sequential_with_context_preservation(self, mock_completion):
        ...
        assert all(a < b for a, b in zip(call_messages, call_messages[1:]))
        assert result is not None, "Agent should return a result"
    ...
    @patch('litellm.completion')
    def test_sequential_tool_call_behavior(self, mock_completion):
        """Test that sequential tool calling handles transient errors appropriately."""
        # Test the current behavior when an API error occurs
        ...
        # Current behavior: returns None when there's an error
        # This documents the existing behavior rather than enforcing retry logic
