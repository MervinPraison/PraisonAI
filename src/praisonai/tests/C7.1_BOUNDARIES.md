...
**Deferred:** ~~C8.4 full `main.py` decomposition~~ — **complete 2026-07-03** (see C8.4 sign-off below).
...
- [x] Legacy `main.py` physical decomposition (C8.4)

## C8.4 sign-off (2026-07-03)

C8.4 `main.py` thinning is **complete**.

| Metric | Before C8.4 | After C8.4 |
|--------|-------------|------------|
| `praisonai_code/cli/main.py` lines | 7,492 | **162** (≤300 gate) |
| PraisonAI class location | inline in main.py | `cli/legacy/praison_ai.py` |
| Domain modules | 0 | 8 wrapper + 2 code legacy modules |
| Dead serve/call/replay branches | present | removed |
| Gateway/bot legacy routing | broken relative import | bridge via `legacy_dispatch.py` |

**Module map:**

| Module | Package |
|--------|---------|
| `cli/main.py` | praisonai-code (thin re-exports) |
| `cli/legacy/env_security.py` | praisonai-code |
| `cli/legacy/praison_ai.py` | praisonai-code |
| `cli/legacy/subcommand_handlers.py` | praisonai |
| `cli/legacy/workflow_commands.py` | praisonai |
| `cli/legacy/direct_prompt.py` | praisonai |
| `cli/legacy/interactive_legacy.py` | praisonai |
| `cli/legacy/dispatch/argparse_builder.py` | praisonai |
| `cli/legacy/dispatch/legacy_dispatch.py` | praisonai |

**Future:** `praisonai-bot` manifest in [`PRAISONAI_BOT_MANIFEST.md`](PRAISONAI_BOT_MANIFEST.md).
