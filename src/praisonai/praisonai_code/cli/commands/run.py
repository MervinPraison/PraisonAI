...
from praisonai.cli.state.identifiers import get_current_context
...
def run_main(
    ...
    # Emit start event
    from praisonai.cli.output.event_bridge import SCHEMA_VERSION
    output.emit_start(
    ...
    # and `--no-checkpoint`.
    if is_file:
        from praisonai.cli.state.identifiers import get_current_context as _get_ctx
...
def _run_from_file(
    ...
    try:
        ...
        if continue_session or session or fork:
            from praisonai.cli.state.project_sessions import find_last_session, session_exists_anywhere
...
def _run_prompt(
    ...
    try:
        ...
        if output_mode == "actions":
            ...
            from praisonai.cli.state.project_sessions import build_cli_memory_config, apply_cli_session_continuity
            ...
            from praisonai.cli.output.event_bridge import attach_bridge, detach_bridge
        ...
        from praisonai.cli.output.event_bridge import StreamEventBridge
