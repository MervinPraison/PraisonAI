                ...
                while True:
                    ...
                    if tools:
                        ...
                        if logging.getLogger().getEffectiveLevel() == logging.DEBUG:
                            total_time = time.time() - start_time
                            logging.debug(f"Agent.achat completed in {total_time:.2f} seconds")
                        # Apply guardrail validation for tool response
                        try:
                            validated_response = self._apply_guardrail_with_retry(result, original_prompt, temperature, tools)
                            return validated_response
                        except Exception as e:
                            logging.error(f"Agent {self.name}: Guardrail validation failed: {e}")
                            # Rollback chat history on guardrail failure
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
                ...
                # Catch any exceptions that escape the while loop
                display_error(f"Unexpected error in achat: {e}", console=self.console)
                # Rollback chat history
                self.chat_history = self.chat_history[:chat_history_length]
                return None
        except Exception as e:
