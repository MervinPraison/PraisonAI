...
{{"name": "tool_name", "arguments": {{"arg1": value1, "arg2": value2}}}}"""
    ...
    def __init__(
        ...
        **extra_settings
        ...
        # Current-agent attribution for token tracking is stored in a ContextVar
        # so concurrent agents that share one LLM instance (or interleaved async
        # tasks) do not overwrite each other's attribution: each task/context
        # reads and writes its own value. A plain attribute would be clobbered by
        # the last set_current_agent() caller (issue #5052).
...
Output MUST be JSON with 'reflection' and 'satisfactory'.
    ...
    def current_agent_name(self) -> Optional[str]:
        """The agent name attributed to the current execution context.

        Backed by a :class:`contextvars.ContextVar` so concurrent agents that
        share one LLM instance keep independent attribution — each async task
        reads the value it set, never a sibling task's overwrite (issue #5052).
        """
    ...
    def set_current_agent(self, agent_name: Optional[str]):
        """Set the current agent name for token tracking (context-local)."""
        self._current_agent_name_var.set(agent_name)

    def __deepcopy__(self, memo):
        """Deep-copy the LLM, giving the clone a fresh attribution ContextVar.

        ``contextvars.ContextVar`` is not copyable (``TypeError: cannot pickle
        '_contextvars.ContextVar'``) and its value is runtime/task-local, so a
        clone must own an independent var rather than share the original's
        (issue #5052). All other attributes are deep-copied normally.
        """
        cls = self.__class__
