...
class ContextCompactionPolicy:
    """
    Policy for proactive context overflow protection.
    
    Replaces opt-in reactive string-matching with proactive token-budget
    management that triggers before LLM calls fail.
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ContextCompactionPolicy":
        """Create policy from dictionary."""
        return cls(**data)


# Pre-defined policy presets
CONSERVATIVE_POLICY = ContextCompactionPolicy(
    trigger_at=0.80,
...
BALANCED_POLICY = ContextCompactionPolicy(
    trigger_at=0.90,
    strategy=CompactionStrategy.DROP_OLDEST_TOOLS,
    preserve_last_n_turns=5,
...
AGGRESSIVE_POLICY = ContextCompactionPolicy(
    trigger_at=0.95,
...
def get_default_policy() -> ContextCompactionPolicy:
