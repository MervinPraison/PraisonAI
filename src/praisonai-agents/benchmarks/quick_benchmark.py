...
Examples:
  ...
  python benchmarks/quick_benchmark.py --all     # All details
    ...
    print()
    if ratio > 1:
        print(f"  → PraisonAI is {ratio:.2f}x slower ({diff:.2f}μs more)")
    else:
        print(f"  → PraisonAI is {1/ratio:.2f}x faster ({-diff:.2f}μs less)")
    ...
    for name, p_val, a_val, better in metrics:
        if better == "lower":
            winner = "PraisonAI ✓" if p_val <= a_val else "Agno ✓"
            if p_val <= a_val:
                praison_wins += 1
            else:
                agno_wins += 1
