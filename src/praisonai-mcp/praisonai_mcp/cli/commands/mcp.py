...
def mcp_remove(
    ...
    get_user_config_path = paths.get_user_config_path
    loader_mod = configuration_loader_helpers()
    _load_toml, _save_toml = loader_mod._load_toml, loader_mod._save_toml
...
def mcp_auth(
    ...
    MCPRemoteConfig = configuration_schema().MCPRemoteConfig
...
_HOST_EXTRA = {"allow_extra_args": True, "ignore_unknown_options": True}


def _run_host_command(command: str, ctx: typer.Context) -> None:
    from praisonai_mcp.mcp_server.cli import handle_mcp_command

    raise typer.Exit(handle_mcp_command([command, *ctx.args]))


@app.command("serve", context_settings=_HOST_EXTRA)
def mcp_serve_host(ctx: typer.Context) -> None:
    """Start the PraisonAI MCP server host (stdio or http-stream)."""
    _run_host_command("serve", ctx)


@app.command("list-tools", context_settings=_HOST_EXTRA)
def mcp_list_tools_host(ctx: typer.Context) -> None:
    """List tools exposed by the MCP server host."""
    _run_host_command("list-tools", ctx)


@app.command("doctor", context_settings=_HOST_EXTRA)
def mcp_doctor_host(ctx: typer.Context) -> None:
    """Check MCP server host health and configuration."""
    _run_host_command("doctor", ctx)


@app.command("serve-recipe", context_settings=_HOST_EXTRA)
def mcp_serve_recipe_host(ctx: typer.Context) -> None:
    """Serve a recipe as an MCP server."""
    _run_host_command("serve-recipe", ctx)
