...
def mint_fresh_link(ttl: int = 600) -> str:
    ...
    """
    try:
        from praisonai_bot.gateway.magic_link import MagicLinkStore
...
app = typer.Typer(help="Mint gateway magic links", no_args_is_help=True)


@app.command("mint-link")
def mint_link_cmd(
    ttl: int = typer.Option(600, "--ttl", help="Link time-to-live in seconds"),
) -> None:
    """Mint a fresh magic link for gateway authentication."""
    class _Args:
        pass
