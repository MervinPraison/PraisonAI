...
def _generate_bot_yaml_multi_channel(channels: Dict[str, Dict], agent_name: str = "assistant", agent_instructions: str = "") -> str:
    """Generate bot.yaml content for multiple channels.
    
    Args:
        channels: Dict of channel_key -> {platform, role, env_var, token, info}
        agent_name: Default agent name
        agent_instructions: Default agent instructions
    """
    ...
    lines = ["# Generated by `praisonai onboard`. Multi-channel configuration.",
             "# Compatible with both `praisonai bot start` and `praisonai gateway start`.",
    ...
    # Legacy single-agent block for backward compatibility
    lines.append("agent:")
    ...
    # Multi-agent block with agents for each role
    lines.append("agents:")
    agents_added = set()
    for channel_key, channel in channels.items():
        role = channel["role"]
        if role not in agents_added:
            lines.append(f"  {role}:")
            if role == "assistant":
                lines.append(f'    instructions: "{instructions_escaped}. You can search the web, manage schedules, and remember past conversations. Use your tools proactively."')
            else:
                lines.append(f'    instructions: "You are a {role} agent. {instructions_escaped} Use your tools proactively to help with {role}-related tasks."')
            lines.append('    model: gpt-4o-mini')
            lines.append("    memory: false")
            lines.append("")
            agents_added.add(role)

    # Channels section with multiple channels per platform supported
    ...
    for channel_key, channel in channels.items():
        ...
        lines.append("    # auto_approve_tools: true")
        lines.append("                               # Chat bots cannot show CLI approval prompts; safe tools auto-approved by default.")
...
def _generate_bot_yaml(platforms: List[str], agent_name: str = "assistant", agent_instructions: str = "") -> str:
    ...
    instructions_escaped = instructions.replace('\\', '\\\\').replace('"', '\\"')  # Escape quotes and backslashes
...
class OnboardWizard:
    ...
    def run(self) -> None:
        ...
        # Step 1: Platform and channel selection
        console.print("\n[bold]Step 1: Configure your bot channels[/bold]\n")
        console.print(
        ...
        self.selected_platforms = [first_platform]
        
        # Configure first channel
        self._configure_channel(console, Prompt, first_platform, is_first=True)
        
        # Ask for additional channels
        while True:
            add_another = _confirm_ask(
                Confirm,
                f"\nAdd another bot channel?",
                default=False,
            )
            if not add_another:
                break
        ...
        for channel_key, channel in self.channels.items():
            if channel["token"]:
                console.print(f"  Testing {channel_key}...", end=" ")
                try:
                    result = asyncio.run(self._probe_channel(channel))
                    if result.ok:
        ...
        yaml_content = _generate_bot_yaml_multi_channel(
            self.channels,
        ...
        try:
            from praisonai_bot.cli.commands.mint_link import mint_fresh_link
    ...
    def _install_daemon_with_feedback(self, print_fn, config_path: str) -> bool:
        ...
        try:
            ...
            from praisonai_bot.daemon import get_daemon_status, install_daemon
