...
from praisonai_bot.gateway.exec_approval import Resolution, ExecApprovalManager
...
async def test_real_agentic_gateway_approval():
    if not os.environ.get("OPENAI_API_KEY"):
        pytest.skip("OPENAI_API_KEY required for real agentic gateway approval test")
    if not os.environ.get("PRAISONAI_AGENTIC_TEST"):
        pytest.skip("Set PRAISONAI_AGENTIC_TEST=1 to run optional live agentic gateway test")
