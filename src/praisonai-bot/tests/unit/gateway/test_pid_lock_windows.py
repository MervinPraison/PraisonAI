...
def _make_lock():
    return GatewayPIDLock(host="127.0.0.1", port=18789)


def test_is_process_running_systemerror(monkeypatch):
    ...
    assert _make_lock()._is_process_running(12345) is False


def test_is_process_running_valueerror(monkeypatch):
    """ValueError from os.kill must be treated as 'not running', not raised."""
    def bad_kill(pid, sig):
        raise ValueError("invalid pid")

    monkeypatch.setattr(os, "kill", bad_kill)
    assert _make_lock()._is_process_running(-1) is False


def test_is_process_running_dead_process(monkeypatch):
    ...
    assert _make_lock()._is_process_running(99999) is False


def test_is_process_running_alive(monkeypatch):
    """A successful os.kill(pid, 0) reports the process as running."""
    monkeypatch.setattr(os, "kill", lambda pid, sig: None)
    assert _make_lock()._is_process_running(4321) is True
