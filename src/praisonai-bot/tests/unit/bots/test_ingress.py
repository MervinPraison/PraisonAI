...
class TestInboundJournalQuarantine:
    ...
    def test_redelivery_stale_claim_over_cap_routes_to_dlq(self, tmp_path):
        """A stale-claimed poison message quarantined via receive() must
        still reach the DLQ (not only via replay())."""
        from praisonai_bot.bots import InboundDLQ, InboundJournal
        ...
        journal._claim_entry(key)  # attempts = 1 >= max_attempts, still 'claimed'
        time.sleep(1.1)  # claim goes stale

        # Redelivery before replay() runs — receive() must quarantine AND
        # route to the DLQ so operator alerting still fires.
        assert journal.receive("telegram", "bot1", "chat1", "poison", {"text": "boom"}) is None
        assert journal.quarantined_count() == 1
        assert dlq.size() == 1
        assert "Max inbound attempts exceeded" in dlq.list(limit=1)[0].error

    def test_redelivery_pending_over_cap_is_quarantined(self, tmp_path):
        """A 'pending' entry (handler kept raising -> _release_claim) that has
        exhausted its attempts must be quarantined on redelivery, not
        re-offered forever."""
        from praisonai_bot.bots import InboundJournal
        ...
        # Simulate two graceful handler failures: claim increments attempts,
        # _release_claim resets to 'pending'.
        for _ in range(2):
            journal._claim_entry(key)
            journal._release_claim(key)
        assert journal.pending_count() == 1

        # Redelivery must not re-offer the exhausted pending entry.
        assert journal.receive("telegram", "bot1", "chat1", "poison", {"text": "x"}) is None
        assert journal.quarantined_count() == 1
        assert journal.pending_count() == 0

    def test_quarantined_entries_evicted_on_overflow(self, tmp_path):
        """Quarantined entries must be shed by overflow eviction before
        healthy pending entries."""
        from praisonai_bot.bots import InboundJournal

        journal = InboundJournal(
            path=tmp_path / "ingress.sqlite", max_size=2, max_attempts=1
        )

        # Directly journal one quarantined + fill past max_size with pending.
        with journal._connect() as conn:
            conn.execute(
                """INSERT INTO ingress_journal(ts, platform, account, channel_id,
                       message_id, payload, status)
                   VALUES (?, 'telegram', 'bot1', 'chat1', 'poison', '{}', 'quarantined')""",
                (time.time() - 100,),
            )
            conn.commit()

        journal.receive("telegram", "bot1", "chat1", "m1", {})
        journal.receive("telegram", "bot1", "chat1", "m2", {})
        # Third insert triggers overflow eviction (max_size=2). The quarantined
        # row should be evicted, leaving the pending entries intact.
        journal.receive("telegram", "bot1", "chat1", "m3", {})

        assert journal.quarantined_count() == 0
        assert journal.pending_count() >= 2

    def test_quarantined_entries_evicted_by_ttl(self, tmp_path):
        """Quarantined entries older than ttl_seconds are aged out."""
        from praisonai_bot.bots import InboundJournal

        journal = InboundJournal(
            path=tmp_path / "ingress.sqlite", ttl_seconds=1
        )
        with journal._connect() as conn:
            conn.execute(
                """INSERT INTO ingress_journal(ts, platform, account, channel_id,
        ...
        time.sleep(1.1)
        # Any receive() runs _evict_expired_locked first.
        journal.receive("telegram", "bot1", "chat1", "fresh", {})
        assert journal.quarantined_count() == 0
