...
Latin) — otherwise non-Tamil outputs are dropped as `low_script_purity`. Only
`validate` rewrites `--out` atomically and leaves an existing file intact when
zero rows survive.
...
skipped here since agent trajectories are English by construction; pass `qc_cfg`
to `export_trials` to enable a script check for your language. Use `--format
