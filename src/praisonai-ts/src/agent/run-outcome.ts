...
// ============================================================================
// RunTerminal (praisonaiagents/run_outcome.py:319-469)
//
 ...
 * `mergeRunTerminal`, never by mutation (Python: `@dataclass(frozen=True)`).
...
export class RunTerminal {
  ...
  /** Serialise to a plain object (JSON friendly, round-trippable). */
  ...
  /** Rehydrate from a plain object produced by `toDict`. */
  ...
  /** Structural equality over `kind`, `source` and `detail`. */
  equals(other: RunTerminal): boolean {
    return this.kind === other.kind && this.source === other.source && (this.detail ?? null) === (other.detail ?? null);
  }
}
