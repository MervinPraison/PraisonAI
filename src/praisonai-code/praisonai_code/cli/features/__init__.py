"""
CLI Features Module for PraisonAI (agentic terminal product).

This module contains modular CLI feature handlers that extend the base CLI
functionality. Each feature is implemented as a separate module for
maintainability and testability.

Migrated to ``praisonai_code`` as part of the C0–C6 extraction (issue #2512).
Public paths under ``praisonai.cli.features.*`` continue to work via a shim in
the main ``praisonai`` package.
