...
def load_user_module(
    ...
    *,
    ...
    # Preserve any prior binding so a bad user module cannot evict an
    # existing entry and break later imports in the same process.
    _sentinel = object()
    previous = sys.modules.get(name, _sentinel)
...
def load_user_module_strict(module_path: str | Path, *, name: str) -> ModuleType:
    ...
    module = importlib.util.module_from_spec(spec)
    # Preserve any prior binding so a failed load cannot evict an existing
    # entry and break later imports in the same process.
    _sentinel = object()
    previous = sys.modules.get(name, _sentinel)
    sys.modules[name] = module
    try:
