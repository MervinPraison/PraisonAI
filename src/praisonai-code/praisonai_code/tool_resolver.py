...
Usage:
    from praisonai_code.tool_resolver import ToolResolver, resolve_tool
...
if TYPE_CHECKING:  # imported only for type annotations to avoid a runtime cycle
    from praisonai_code.tool_registry import ToolRegistry
...
class ToolResolver:
    ...
    def resolve_all_from_yaml(self, yaml_config: Dict[str, Any]) -> Dict[str, Callable]:
        ...
        # Merge both sections so mixed-migration configs that declare tools
        # under ``agents`` are not skipped when ``roles`` is also present.
        role_configs = []
        for section in (yaml_config.get('roles') or {}, yaml_config.get('agents') or {}):
            if isinstance(section, dict):
                role_configs.extend(section.values())
