...
class GNAPStorageBackend:
    ...
    def _run_git_cmd(self, cmd: List[str], check: bool = True) -> subprocess.CompletedProcess:
        ...
        try:
            # Lazy import GitPython alternative: use subprocess
            full_cmd = ["git"] + cmd
...
if __name__ == "__main__":
    # Test the GNAP storage backend
    import tempfile
    import shutil
    
    with tempfile.TemporaryDirectory() as temp_dir:
        print(f"Testing GNAP backend in: {temp_dir}")
        ...
        test_task = {
            ...
            "status": "pending", 
            "agent": "test_agent",
            "description": "Test task for GNAP backend",
            "created_at": datetime.utcnow().isoformat()
        }
        
        backend.save("test_task_001", test_task)
        
        loaded = backend.load("test_task_001")
        print(f"Saved and loaded task: {loaded}")
        ...
        # Test exists
        exists = backend.exists("test_task_001")
        ...
        print("GNAP backend test completed successfully!")