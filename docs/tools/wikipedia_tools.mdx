...
<Steps>
  ...
  <Step title="Import Components">
    ...
    from praisonaiagents.tools import search, get_summary, get_page, get_random, set_language
    ```
  </Step>
  ...
  <Step title="Create Agent">
    ...
    wiki_agent = Agent(
        ...
        tools=[search, get_summary, get_page, get_random, set_language],
        self_reflect=False
    )
    ```
  </Step>
...
<CardGroup cols={2}>
  <Card title="Wiki Agent" icon="user-robot">
    ...
    ```python
    Agent(tools=[search, get_summary, get_page, get_random, set_language])
    ```
  </Card>
...
researcher_agent = Agent(
    ...
    tools=[search, get_summary, get_page],
    self_reflect=False
)
