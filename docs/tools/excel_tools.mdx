---
title: "Excel Tools"
description: "Tools for working with Excel files in AI agents."
---
...
<CardGroup cols={2}>
  <Card title="Excel Agent" icon="user-robot">
    ...
    ```python
    Agent(tools=[excel, read_excel, write_excel, merge_excel])
    ```
  </Card>
...
```python
from praisonaiagents import Agent, Task, PraisonAIAgents
from praisonaiagents.tools import excel, read_excel, write_excel, merge_excel
...
excel_agent = Agent(
    ...
    tools=[excel, read_excel, write_excel, merge_excel],
    self_reflect=False
)
